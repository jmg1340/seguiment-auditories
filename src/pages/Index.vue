      ...
      <div class="col-md-8 col-12 q-mt-md">
        ...
        >
          <template v-slot:top-right>
            <q-btn dense color="grey-8" no-caps label="Nova auditoria a fer seguiment" @click="novaAuditoria" />
          </template>
          ...
          <template v-slot:body="props">
            <q-tr :props="props">
              ...
              <q-td key="DATA" :props="props" @click="anarAObsMillores(props.key)">
                {{ props.row.dataAuditoria }}
              </q-td>
              <q-td key="CA" :props="props" @click="anarAObsMillores(props.key)">
                {{ props.row.centre }}
              </q-td>

              <q-td key="OBS_pend" :props="props" @click="anarAObsMillores(props.key)">
                {{ props.row.obsPend }}
              </q-td>
              <q-td key="OBS_susp" :props="props" @click="anarAObsMillores(props.key)">
                {{ props.row.obsSusp }}
              </q-td>

              <q-td key="NC_pend" :props="props" @click="anarAObsMillores(props.key)">
                {{ props.row.NCPend }}
              </q-td>
              <q-td key="NC_susp" :props="props" @click="anarAObsMillores(props.key)">
                {{ props.row.NCSusp }}
              </q-td>
              
              <q-td key="ELIMINAR" :props="props">
                <q-btn color="red-5" text-color="white" icon="delete" dense @click="eliminar(props.key)"/>
              </q-td>
    ...
    <q-dialog v-model="activarFormulari" persistent>
      <q-card>
        <q-card-section class="col items-start">
          <q-input class="col q-mb-md " color="brown" stack-label label-color="negative" filled v-model="txtCentre" label="Centre" />
          <q-input class="col" v-model="txtData" stack-label label-color="negative" filled type="date" label="Data" />
        </q-card-section>

        <q-card-actions align="right">
          <q-btn flat label="Cancel" color="primary" v-close-popup />
          <q-btn flat label="Ok" color="primary" v-close-popup @click="afegirRegistre"/>
        </q-card-actions>
      </q-card>
    </q-dialog>
...
export default {
  ...
  data () {
    return {
      columns: [
        { name: 'DATA', align: 'center', label: 'DATA AUDIT.', field: 'dataAuditoria', sortable: true,
        ...
        { name: 'OBS_pend', align: 'center', label: 'OBS pend.', field: 'obsPend', sortable: false,
        headerClasses: 'bg-brown-14 text-brown-1' },
        { name: 'OBS_susp', align: 'center', label: 'OBS susp.', field: 'obsSusp', sortable: false,
        headerClasses: 'bg-brown-14 text-brown-1' },

        { name: 'NC_pend', align: 'center', label: 'NC. pend.', field: 'NCPend', sortable: false,
        headerClasses: 'bg-brown-14 text-brown-1' },
        { name: 'NC_susp', align: 'center', label: 'NC. susp.', field: 'NCSusp', sortable: false,
        headerClasses: 'bg-brown-14 text-brown-1' },

        { name: 'ELIMINAR', align: 'center', label: '', field: 'idx', sortable: false,
        headerClasses: 'bg-brown-14 text-brown-1' },
      ],
  ...
  methods: {
        ...
        if (this.$q.platform.is.electron) {
          ...
          let promesa = new Promise ( (resolve, reject) => {
            ...
            if ( this.carregarAuditories ) {
              console.log("LOCALSTORAGE: has seg_auditories? " + store.has("seg_auditories"))
              if ( store.has("seg_auditories") ) {
                this.$store.commit("example/mutImportarJSON", JSON.parse(store.get("seg_auditories").auditories))
          ...
          console.log("key seg_auditories: " + this.$q.localStorage.getItem("seg_auditories"))

          let promesa = new Promise ( (resolve, reject) => {
            console.log("this.carregarAuditories? " + this.carregarAuditories)
            if ( this.carregarAuditories ) {
              console.log("LOCALSTORAGE: has seg_auditories? " + this.$q.localStorage.has("seg_auditories"))
              if ( this.$q.localStorage.has("seg_auditories") ) {
                this.$store.commit("example/mutImportarJSON", JSON.parse(this.$q.localStorage.getItem("seg_auditories").auditories))
                //this.$store.commit("example/mutImportarJSON", arr);
                resolve ()
              }
            } else {
    ...
    afegirRegistre: function(){
      let obj = { centre: this.txtCentre, dataAuditoria: this.txtData, obs_noConf: [] }
     ...
     eliminar: function(idx) {
      ...
      this.$q.dialog({
        title: 'Confirm',
        message: 'Eliminar auditoria ' + this.auditories[idx].centre.toUpperCase() + '?',
        cancel: true,
        persistent: true
      }).onOk(() => {
  ...
  computed: {
    auditories: function() {
      return this.$store.state.example.auditories.map((val,idx) => ({...val, index: idx}))
    },
